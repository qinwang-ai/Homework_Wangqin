Init: ;程序初始化
    delayTime equ 5000  ;定义 delayTime 代表 50
	org 7c00h  ;程序装载到 7c00h
...
Show:  ;显示字符
	...
	mov es,ax   ;把 ax 的值加载到 es，作为存放字符的基址
	...
	mov [es:bx],ax  ;把 ax 的值放到 [es:bx] （基址+偏移量） 处，字符完成显示
...
data:  ;数据声明部分
	...
	times 510-($-$$) db 0 ; 用0填充扇区的剩余部分（$$=当前节地址）
	db 55h,0aah	; 引导扇区的结束标志，也可以改为：dw 0aa55h
