...
disk.img: boot.bin os.bin process1.com process2.com process3.com process4.com process_wait_key.com usr1.com usr2.com process_semaphore.com
...
#	dd if=usr1.com of=disk.img seek=1
	...
	dd if=process_semaphore.com of=disk.img bs=512 seek=216
...
process_semaphore.elf:process_semaphore.c
	gcc $(C_flags) -c $^ -o $@

process_semaphore.com:process_semaphore.elf osclib_share.o oslib_share.o usrlib.o
